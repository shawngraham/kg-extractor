 ...
 */
/**
 * Direct LLM querying logic matching server configurations with Diagnostics
 */
async function queryLLM(systemPrompt: string, userPrompt: string): Promise<string> {
  let host = (process.env.LOCAL_API_HOST || "http://localhost:2276").trim().replace(/\/$/, "");
  
  if (host.endsWith("/v1")) {
    host = host.slice(0, -3);
  }
  
  const apiKey = process.env.LOCAL_API_KEY || "sk-no-key-required";
  const model = process.env.LOCAL_API_MODEL || "ggml-org/Qwen3-VL-30B-A3B-Instruct-Q8_0-GGUF:Q8_0";
  ...
  try {
    const response = await fetch(endpoint, {
      ...
      body: JSON.stringify({
        ...
        stream: false,
        temperature: 0.1,
        // Ensure standard raw string returns so our regex can clean it reliably
    ...
    // 1. Extract either standard content or reasoning fallback
    const content = message.content || "";
    const reasoning = message.reasoning_content || message.reasoning || "";
    const textToProcess = content.trim() !== "" ? content : reasoning;

    // 2. Strip thinking tags and everything in between them
    // This matches <think>...</think> or <think>...[end of text] if truncated
